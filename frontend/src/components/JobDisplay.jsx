...
const JobDisplay = ({job}) => {
  ...
  return (
    ...
    <h1 className="text-sm text-gray-700 text-left">{job.description.length > 135
    ? job.description.slice(0, 135) + "....."
    : job.description} </h1>
  </div>
   <div className='flex items-center gap-3 mt-4'>
    <Badge className="text-purple-800 font-bold b" variant="ghost">{job?.position} position</Badge>
    <Badge className="text-red-600 font-bold" variant="ghost">{job?.type}</Badge>
    <Badge className="text-green-900 font-bold" variant="ghost">{job?.salary} LPA</Badge>
    </div>
    <div className="flex items-center gap-4 mt-4">
      <Button variant="outline" className="cursor-pointer w-full bg-fuchsia-600 text-white hover:bg-fuchsia-700  text-shadow-white" onClick={()=>navigate(`/description/${job?._id}`)}>Details</Button>
